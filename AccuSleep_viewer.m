function [message] = AccuSleep_viewer(EEG, EMG, SR, epochLen, labels, savepath)
...
switch nargin
    ...
    case 4
        G.labels = [];
    case {5,6}
        if ~isempty(labels) % user tried to pass some labels
            % check whether they have the correct format
            x = struct;
            x.labels = labels;
            if checkLabels(x, 0) % meets certain conditions
                G.labels = x.labels;
                if isrow(G.labels)
                    G.labels = G.labels';
                end
...
[s, t, f] = createSpectrogram(standardizeSR(G.EEG, G.originalSR, G.SR), G.SR, G.epochLen);
...
G.nbins = length(t); % total number of time bins in the recording
...
if ~isempty(G.labels)
    if length(G.labels) ~= G.nbins
        ...
        clear global G
        message = 'ERROR: Length of labels file does not match EEG/EMG';
        return
    end  
...
showFreqs = find(f <= 30); % only show frequencies under 30 Hz
...
specSample = reshape(s(sampleBins,showFreqs),1,length(sampleBins)*length(showFreqs));
...
% plot spectrogram
caxis(G.A3,G.caxis1);
imagesc(G.A3,G.specTh, f(showFreqs), G.spectrogram',G.caxis1);
axis(G.A3, 'xy')
...
function closeReq(~,~) % make sure to clear the global variable when quitting
global G;
if G.unsavedChanges
